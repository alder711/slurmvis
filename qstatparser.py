...
def parseDataDict(raw_data):
    """ Take raw qstats data and parse it into a Python dictionary.
    """
    result = {}
    with StringIO(raw_data) as f:
        # First line = cluster's name
        curr_line = f.readline().rstrip()
        # Err if first line not valid
        if "CLUSTERNAME" not in curr_line:
...
def parseData(indata):
    """ Parse indata as qstat information into json,
        returning the result.
    """
    return json.dumps(parseDataDict(indata))
